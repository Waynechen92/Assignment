...
################################################################################
    ...
    def make_variables(self, horizon: int) -> None:
        ...
        for n in range(horizon+1): # proposition has range [0,k]
            for p in self.problem.propositions: # there should be n*(K+1) proposition
                self.proposition_fluent_codes[(p,n)] = self.new_cnf_code(n,str(p),p)
            if n != horizon: # action range [0,k-1]
                for a in self.problem.actions: # there should be n*k action
                    self.action_fluent_codes[(a,n)] = self.new_cnf_code(n,str(a),a)
    ...
    def make_initial_state_and_goal_axioms(self, horizon: int) -> None:
        ...
        """ *** YOUR CODE HERE *** """
        # All propositions that are contained in the initial state and only these must hold at step 0
        ...
        for goal in self.problem.goal:
            self.add_clause([self.proposition_fluent_codes[(goal, horizon)]], "goal")
    ...
    def make_precondition_and_effect_axioms(self, horizon: int) -> None:
        ...
        for n in range(horizon):
            for a in self.problem.actions:
                act_t = self.action_fluent_codes[(a,n)]
                for p in a.preconditions: # Its preconditions must be true at step t. a -> p = not a or p            
                    self.add_clause([self.proposition_fluent_codes[(p,n)],-act_t], "pre")
                for eff in a.pos_effects: # Its positive effects are true at step t + 1
                    self.add_clause([-act_t,self.proposition_fluent_codes[(eff,n+1)]], "eff")
                for eff in a.neg_effects: # Its negative effects are false at step t + 1
                    self.add_clause([-act_t,-self.proposition_fluent_codes[(eff,n+1)]], "eff")
    ...
    def make_explanatory_frame_axioms(self, horizon: int) -> None:
        ...
        for t in range(1, horizon+1): 
            for p in self.problem.propositions: # p_t1 is current state, p_t is previous state
                p_t1 = self.proposition_fluent_codes[(p,t)]
                p_t = self.proposition_fluent_codes[(p,t-1)]
                true_clause = [p_t, -p_t1]
                false_clause = [-p_t, p_t1]
        # If a fluent becomes true, then an action must have added it   
                for pos in p.pos_effects:
                    act_t = self.action_fluent_codes[(pos, t-1)]
                    true_clause.append(act_t)
                self.add_clause(true_clause, "frame")
                
                for neg in p.neg_effects:
                    act_t = self.action_fluent_codes[(neg, t-1)]
                    false_clause.append(act_t)
                self.add_clause(false_clause, "frame")
    ...
    def make_serial_mutex_axioms(self, horizon: int) -> None:
        ...
        for t in range(horizon):
            for i in range(len(self.problem.actions)):
                a1 = self.problem.actions[i]
                a_t1 = self.action_fluent_codes(a1, t)
                # pos1 = set(a1.pos_effects)
                # neg1 = set(a1.neg_effects)
                for j in range(i+1, len(self.problem.actions)):
                    a2 = self.problem.actions[j]
                    if a1 != a2:
                        a_t2 = self.action_fluent_codes(a2, t)
                        # pos2 = set(a2.pos_effects)
                        # neg2 = set(a2.neg_effects)
                        # if pos1.isdisjoint(neg2):
                        #     if neg1.isdisjoint(pos2):
                    # actions cannot occur in parallel
                        ser_mutex_caluse = (-a_t1, -a_t2)
                        self.add_clause(ser_mutex_caluse, "mutex")
    ...
    def make_interference_mutex_axioms(self, horizon: int) -> None:
        ...
        for t in range(horizon):
            duplicate = set() # avoid duplicate clauses
            for i in range(len(self.problem.actions)):
                a1 = self.problem.actions[i]
                a_t1 = self.action_fluent_codes[(a1, t)]
                # pos1 = set(a1.pos_effects)
                # neg1 = set(a1.neg_effects)
        #  p in EFF-(a1) 
                for p in a1.neg_effects:
                    #  p in PRE(a2)
                    for a2 in p.preconditions:
                        if a1 != a2 and (a1, a2) not in duplicate:
                            a_t2 = self.action_fluent_codes[(a2, t)]
                            # pos2 = set(a2.pos_effects)
                            # neg2 = set(a2.neg_effects)
                            # if pos1.isdisjoint(neg2):
                            #     if neg1.isdisjoint(pos2):
                            self.add_clause((-a_t1, -a_t2), "mutex")
                            duplicate.add((a1, a2))
    ...
    def make_reachable_action_axioms(self, horizon: int) -> None:
        """ Make unit clauses preventing actions from being executed before they
            ...
            For example, if self.problem.action_first_step[a1] == 5, then
            you would introduce clauses stopping a1 from being executed at
            steps 0..4.
        ...
        for action in self.problem.actions:
            #  clauses stopping a1 from being executed at steps 0~s  
            for s in range(self.problem.action_first_step[action]):
                a_t = self.action_fluent_codes[(action, s)]
                self.add_clause([-a_t], "reach")
    ...
    def make_fluent_mutex_axioms(self, horizon: int) -> None:
        ...
        """ *** YOUR CODE HERE *** """
      # the dictionary doesn't contain steps greater than n
        n = max(k for k, v in self.problem.fluent_mutex.items())
     # mutex relationships larger than the horizon should be ignored.            
        if n > horizon:
            step = horizon
        else:
            step = n
    #  step 0 not consider, from 1 to k
        # for m in range(1, step+1):
        for s in range(1, step+1):
    # if current step bigger than n, querying the dictionary with step n.
            s1 = s
            if s > n:
                s = n
            for mutex in self.problem.fluent_mutex[s]:
                p_t1 = self.proposition_fluent_codes[(mutex[0], s1)]
                p_t2 = self.proposition_fluent_codes[(mutex[1], s1)]
                self.add_clause([-p_t1,-p_t2], "fmutex")
    ...
    def build_plan(self, horizon: int) -> None:
        ...
        """ *** YOUR CODE HERE *** """
        # create n empty [] for steps. if the empty is not replaced with information, then there is no valid actions
        for n in range(horizon):
            self.plan.append([])
        for m in self.true_vars:
            if isinstance(self.cnf_code_objects[m], Action):
                steps = self.cnf_code_steps[m]
                objects = self.cnf_code_objects[m]
                self.plan[steps].append(objects)
